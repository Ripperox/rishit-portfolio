...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- internal visitor log: updated on EVERY visit so the dashboard is live;
    //      deduped by person (fresh lastSeen bumps them to the top), capped, plus a
    //      raw chronological hit feed. Org is resolved once and cached, so repeat
    //      visits are cheap. ----
    try {
      const existing = parseJSON(await redis.hget('visits:people', h))
      const ua = parseUA(req.headers['user-agent'] || '')
      const ref = (req.headers['referer'] || '').slice(0, 200) || null
      ...
      // raw chronological hit feed (every visit, deduped or not)
      await redis.lpush(
        'visits:hits',
        JSON.stringify({ t: Date.now(), city: you.city, region: you.region, country: you.country, org: org || null, device: ua.device, browser: ua.browser, ref, isNew: !!isNew }),
      )
      await redis.ltrim('visits:hits', 0, 199)

      // bound the people set — evict the oldest beyond the cap
